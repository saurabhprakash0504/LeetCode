...
// Adds a run to this collection
    ...
    public int personalBest() {
        ...
        for (Run r : runs) {
            ...
            if (r.complete == true) {
                ...
                val = Integer.min(r.getRunTime(), val);
    ...
    //TODO:: Implement this code (2)
    public int bestOfBests() {
        int obstacleCount = course.obstacleCount;
        int[] bestTimes = new int[obstacleCount];

        // Initialize with large values
        Arrays.fill(bestTimes, Integer.MAX_VALUE);

        // For each run, update best time per obstacle
        for (Run r : runs) {
            for (int i = 0; i < r.obstacleTimes.size(); i++) {
                bestTimes[i] = Math.min(bestTimes[i], r.obstacleTimes.get(i));
            }
        }
    ...
    //TODO:: Implement this code (3)

    public double chanceOfPersonalBest(Run inProgressRun) {
        int trials = 10000;
        int obstacleCount = course.obstacleCount;

        // Collect all times per obstacle index across ALL runs (including incomplete)
        List<List<Integer>> obstacleTimePool = new ArrayList<>();
        for (int i = 0; i < obstacleCount; i++) {
            obstacleTimePool.add(new ArrayList<>());
        }

        for (Run r : runs) {
            for (int i = 0; i < r.obstacleTimes.size(); i++) {
                obstacleTimePool.get(i).add(r.obstacleTimes.get(i));
            }
        }

        // Personal best among completed runs
        ...
        for (int t = 0; t < trials; t++) {
            ...
            // Add completed obstacle times from the in-progress run
            simulatedTime += inProgressRun.getRunTime();

            // Fill in remaining obstacles using random samples
            for (int i = inProgressRun.obstacleTimes.size(); i < obstacleCount; i++) {
                List<Integer> pool = obstacleTimePool.get(i);
                int randomIndex = (int) (Math.random() * pool.size());
                simulatedTime += pool.get(randomIndex);
            }

            // Check if simulated run beats or matches personal best
            if (simulatedTime <= personalBest) {
                successes++;
            }
        }
...
// Run 4: 5 5 3 (13 seconds, but run is incomplete)
    ...
    public static void testBestVal() {
...
// Run 4: 5 5 3 (13 seconds, but run is incomplete)
        ...
        assert runCollection.bestOfBests() == 15 :
                ...
                "Best should be 15, was " + runCollection.bestOfBests();
